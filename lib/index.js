...
function addHookLegacy(hook, opts = {}) {
  let reverted = false;
  const loaders = [];
  const oldLoaders = [];
  let exts;
...
function addHookModern(hook, opts = {}) {
  let reverted = false;
  ...
  exts.forEach((ext) => {
    ...
    oldLoaders[ext] || originalJSLoader;
  });
  ...
  const hooks = Module.registerHooks({
    load(url, context, nextLoad) {
      ...
      if (reverted || !url.startsWith('file://')) {
        return result;
      }
      ...
      return Object.assign({}, result, { source: newCode });
    },
  });
  ...
  return function revert() {
    ...
    hooks.deregister();
    exts.forEach((ext) => {
      // Only restore if the slot still holds the value we installed. If some
      // other code has since patched Module._extensions[ext], leave it alone
      // rather than clobbering their loader.
      if (Module._extensions[ext] === loaders[ext]) {
        if (oldLoaders[ext]) {
          Module._extensions[ext] = oldLoaders[ext];
        } else {
 ...
 */
const addHook = hasRegisterHooks ? addHookModern : addHookLegacy;
